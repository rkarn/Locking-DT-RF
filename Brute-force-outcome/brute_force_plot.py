...
for i,dt_name in enumerate(Acc_dict.keys()):
    ax[i].plot(np.arange(1,len(Acc_dict[dt_name])+1), Acc_dict[dt_name], marker=m[i], color = c[i], fillstyle='none', label='Incorrect key')
    ax[i].plot(np.arange(1,len(Acc_dict[dt_name])+1), correct_key_acc[i] * len(Acc_dict[dt_name]) , linestyle='dashed', color = c[i], fillstyle='none', label='Correct key')
